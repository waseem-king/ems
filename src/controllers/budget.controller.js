...
class BudgetController{
    setMyBudget = asyncHandler( async (req, res)=>{
     ...
     getMyBudget = asyncHandler( async (req, res)=>{
        const response = await newBudgetServices.getBudget(req.user.id, req.params.id)
        res.status(200).json({ status:"success", data:response});
    })
     updateMyBudget = asyncHandler( async (req, res)=>{
        const response = await newBudgetServices.updateBudget(req.user.id, req.params.id, req.body)
        res.status(200).json({ status:"success", data:response});
    })
...
/////////////////////////////////// BUDGET CONTROLLER AGGREGATIONS /////////////////////////////////////////
    budgetDashboard = asyncHandler( async (req, res)=>{
        ...
        const response = await newBudgetServices.getBudgetDashboard(req.user.ownerType, req.user.id)
        res.status(200).json({ status:"success", data:response});
    })
